...
led_fill). Weights from combo2_weights.bin at 0x50000."""
...
elf=os.path.join(HERE,"combo2.elf"); ld=os.path.join(HERE,"combo2_link.ld")
...
subprocess.run([GCC,"-mcpu=cortex-m4","-mthumb","-mfpu=fpv4-sp-d16","-mfloat-abi=softfp",
  ...
  "-T",ld,"-o",elf,os.path.join(HERE,"combo2_nn.c")],check=True)
...
subprocess.run([OBJCOPY,"-O","binary","-j",".text",elf,os.path.join(HERE,"combo2_lo.bin")],check=True)
code=open(os.path.join(HERE,"combo2_lo.bin"),"rb").read()
weights=open(os.path.join(HERE,"combo2_weights.bin"),"rb").read()
...
open(os.path.join(HERE,"combo2_app_kb.txt"),"w").write(str(app_kb))
