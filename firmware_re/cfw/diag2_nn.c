...
void render(uint8_t *fb, int len){
    ...
    /* ---- flash-write integrity map across the 306KB weight blob ---- */
    static const uint32_t ADDR[8]={0x50000u,0x60000u,0x70000u,0x80000u,0x88000u,0x90000u,0x94c00u,0x9a000u};
    static const uint8_t  EXP[8] ={0xe2,0x1a,0xf5,0x41,0x05,0xef,0xed,0xe8};
    uint32_t *pp=(uint32_t*)fb; for(int i=0;i<0x2000/4;i++) pp[i]=0;
    for(int k=0;k<8;k++){
        int ok = (*(volatile uint8_t*)ADDR[k] == EXP[k]);
        draw_char(fb, 4+k*31, 6, ok?24:13, 0xF, 3);   /* Y/N per checkpoint */
    }
    draw_char(fb, 4, 40, M[2], 0x8, 2);                /* last typed char */
    for(int j=0;j<2;j++){ PLED[j]=0xFF; }
    LEDT[0]=LEDT[0]+1;
    HUE[0]=PREDS[0];
}
