...
led_fill). Weights from combo3_weights.bin at 0x50000."""
...
elf=os.path.join(HERE,"diag3b.elf"); ld=os.path.join(HERE,"diag3b.ld")
...
subprocess.run([GCC,"-mcpu=cortex-m4","-mthumb","-mfpu=fpv4-sp-d16","-mfloat-abi=softfp",
  ...
  "-T",ld,"-o",elf,os.path.join(HERE,"diag3_nn.c")],check=True)
...
subprocess.run([OBJCOPY,"-O","binary","-j",".text",elf,os.path.join(HERE,"diag3b_lo.bin")],check=True)
code=open(os.path.join(HERE,"diag3b_lo.bin"),"rb").read()
...
open(os.path.join(HERE,"diag3b_kb.txt"),"w").write(str(app_kb))
