...
elf=os.path.join(HERE,"diag.elf"); ld=os.path.join(HERE,"diag_link.ld")
...
subprocess.run([GCC,"-mcpu=cortex-m4","-mthumb","-mfpu=fpv4-sp-d16","-mfloat-abi=softfp",
  ...
  "-T",ld,"-o",elf,os.path.join(HERE,"diag_nn.c")],check=True)
...
subprocess.run([OBJCOPY,"-O","binary","-j",".text",elf,os.path.join(HERE,"diag_lo.bin")],check=True)
code=open(os.path.join(HERE,"diag_lo.bin"),"rb").read()
...
open(os.path.join(HERE,"diag_app_kb.txt"),"w").write(str(app_kb))
