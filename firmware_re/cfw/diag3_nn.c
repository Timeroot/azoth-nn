...
void render(uint8_t *fb, int len){
    ...
    uint8_t  wb   = *(volatile uint8_t*)0x50000u;                 /* expect 0xD0 */
    float    emb0 = *(volatile float*)(0x50000u + OFF_EMB);        /* expect ~0.225 */
    volatile float *sc=(volatile float*)0x2003b000u; sc[0]=1.5f; sc[300]=2.5f;
    int wok=(wb==0xD0), eok=(emb0>0.22f && emb0<0.23f), sok=(sc[0]==1.5f && sc[300]==2.5f);
    /* also: is the hidden state finite & non-degenerate after steps? */
    float hs=HID[0]; int hok=(hs==hs) && (hs>-50.0f && hs<50.0f) && (hs>0.0001f||hs<-0.0001f);
    uint32_t *pp=(uint32_t*)fb; for(int i=0;i<0x2000/4;i++) pp[i]=0;
    draw_char(fb, 4,   40, M[2], 0x8, 2);
    draw_char(fb, 20,  16, wok?24:13, 0xF, 4);   /* W */
    draw_char(fb, 68,  16, eok?24:13, 0xF, 4);   /* E */
    draw_char(fb, 116, 16, sok?24:13, 0xF, 4);   /* S */
    draw_char(fb, 164, 16, hok?24:13, 0xF, 4);   /* H hidden-state ok */
    draw_char(fb, 212, 16, PREDS[0], 0xF, 4);    /* P top pred */
