...
void render(uint8_t *fb, int len){
    ...
    volatile float *g1=(volatile float*)0x2003b000u; g1[0]=1.5f; g1[300]=2.5f;
    volatile float *g2=(volatile float*)0x2003bc00u; g2[0]=3.5f; g2[200]=4.5f;
    int s1=(g1[0]==1.5f && g1[300]==2.5f);
    int s2=(g2[0]==3.5f && g2[200]==4.5f);
    for(int i=0;i<H;i++) HID[i]=0.0f;
    gru_step(SYM_SP);
    float gi0=GI[0], gh0=GH[0];
    int giok=(gi0==gi0)&&gi0>-1e3f&&gi0<1e3f;
    int ghok=(gh0==gh0)&&gh0>-1e3f&&gh0<1e3f;
    int hnz=0; for(int i=0;i<H;i++){ float v=HID[i]; if(v==v && (v>0.01f||v<-0.01f)) hnz++; }
    int hok=(hnz>3);
    predict(SYM_SP);
    uint32_t *pp=(uint32_t*)fb; for(int i=0;i<0x2000/4;i++) pp[i]=0;
    draw_char(fb, 4,   16, s1?24:13, 0xF, 4);
    draw_char(fb, 52,  16, s2?24:13, 0xF, 4);
    draw_char(fb, 100, 16, giok?24:13, 0xF, 4);
    draw_char(fb, 148, 16, ghok?24:13, 0xF, 4);
    draw_char(fb, 196, 16, hok?24:13, 0xF, 4);
    draw_char(fb, 4,  44, PREDS[0], 0x8, 2);
